...
function rebus(){
    var g = $('.days').val();
    var d =  $('.area').val();
    var z = moment(d, "DD.MM.YYYY").isoAddWeekdaysFromSet(g, [1,2,3,4,5],['2017-01-02','2017-01-03','2017-01-04','2017-01-05','2017-01-06','2017-02-23','2017-02-24','2017-03-08','2017-05-01','2017-05-08','2017-05-09','2017-06-12','2017-11-06','2019-01-02','2019-01-03','2019-01-04','2019-01-05','2019-01-06','2019-02-23','2019-02-24','2019-03-08','2019-05-01','2019-05-08','2019-05-09','2019-06-12','2019-11-06','2019-01-02','2019-01-03','2019-01-04','2019-01-05','2019-01-06','2019-02-23','2019-02-24','2019-03-08','2019-05-01','2019-05-08','2019-05-09','2019-06-12','2019-11-06','2020-01-02','2020-01-03','2020-01-04','2020-01-05','2020-01-06','2020-02-23','2020-02-24','2020-03-08','2020-05-01','2020-05-08','2020-05-09','2020-06-12','2020-11-06']);
    dateA = z.format('DD.MM.YYYY');
    $('.ogo').html(dateA);
    var c = z.format('D MMMM'); //LL with year
    $('.day-name').html(c);
    //var r = z.format('ll');
    var toend = $('.days').val();
    var length = String(toend).length-1;
    var lastnum = String(toend)[length];
    if(toend>=10 && toend<=20) {
        $('.days-descr').html("Рабочих дней");
    } else {
        if (lastnum == 1) {
          $('.days-descr').html("Рабочий день");
        } else if (lastnum >=2 && lastnum <=4){
          $('.days-descr').html("Рабочих дня");
        } else {
          $('.days-descr').html("Рабочих дней");
        }
    }
    // DAYS
    var day = z.format('D');
    var dayTo10 = ['первое', 'второе', 'третье', 'четвертое', 'пятое', 'шестое', 'седьмое', 'восьмое', 'девятое', 'десятое'];
    var dayTo20 = ['один', 'две', 'три', 'четыр', 'пят', 'шест', 'сем', 'восем', 'девят'];
    var dayText;
    if (day.length === 1 || day==10){
      dayText = dayTo10[day-1];
    } else{
      if(day>=11 && day<=19){
        dayText=dayTo20[day-11]+"надцатое";
      } else if (day == 20){
        dayText="двадцатое";
      } else if (day>=21 && day<=29){
        dayText="двадцать "+dayTo10[day-21];
      } else if (day == 30){
        dayText="тридцатое";
      } else if (day == 31){
        dayText="тридцать "+dayTo10[day-31];
      }
    }
    // YEARS
    var yearText;
    var year = z.format('YY'); //last num year
    var dayTo2021 = ['первого', 'второго', 'третьего', 'четвертого', 'пятого', 'шестого', 'седьмого', 'восьмого', 'девятого', 'десятого'];
    if (year <= 10) {
      yearText=" две тысячи "+dayTo2021[year-1]+" года";
    }
    if (year >= 11 && year <= 19){
      yearText=" две тысячи "+dayTo20[year-11]+"надцатого года";
    }
    if (year == 20){
      yearText=" две тысячи двадцатого года"
    }
    if (year >= 21 && year <= 29){
      yearText=" две тысячи двадцать "+dayTo2021[year-21]+" года";
    }
    if (year == 30){
      yearText=" две тысячи тридцатого года"
    }
    if (year >= 31 && year <= 39){
      yearText=" две тысячи тридцать "+dayTo2021[year-31]+" года";
    }
    if (year == 40){
      yearText=" две тысячи сорокового года"
    }
    if (year >= 41 && year <= 49){
      yearText=" две тысячи сорок "+dayTo2021[year-41]+" года";
    }
    if (year == 50){
      yearText=" две тысячи пятидесятого года"
    }
    // RESULT
    $('.ogo-letter').html(dayText+" "+z.format('MMMM')+yearText);



    //changed date in calendar
    var startDate = moment(d, "DD.MM.YYYY").format('YY');

//сделать до 20го года
    if(year < 17 < 20 && startDate < 17 < 20){
       $('.ogo-wrap').addClass("ogo-wrap--danger");
       $('.info').html("!");
       $('.info').addClass("info--animate");
      $('.info').click(function() {
        $('.attention').addClass("attention--show");
        $('.holydays').removeClass("holydays--show");
        $('.attention__weekends').hide();
        $('.attention__year').html("2017");
        $('.attention__info').html("праздничные дни могут быть некорректны");
      });
    }

    if(year == 17 && startDate == 17){
      $('.ogo-wrap').removeClass("ogo-wrap--danger");
      $('.info').html("?");
      $('.info').removeClass("info--animate");
      $('.info').click(function() {
        $('.holydays').addClass("holydays--show");
        $('.attention').removeClass("attention--show");
      });
    }
    if(year > 20 || startDate > 20){
      $('.ogo-wrap').addClass("ogo-wrap--danger");
      $('.info').html("!");
      $('.info').addClass("info--animate");
      $('.info').click(function() {
      $('.attention').addClass("attention--show");
      $('.holydays').removeClass("holydays--show");
      $('.attention__weekends').show();
      $('.attention__year').html("2020");
      $('.attention__info').html("не учитываются праздничные дни");
      });
    }
...
$(".area").inputmask("d.m.y",{ "oncomplete": function(){
  rebus();
} });

$('.days').keyup(function rav (){
  rebus();
});

$('.key').click(function() {
  if ($('.days').val().length !== 3){
    var num = $(this).text();
    $('.days').val($('.days').val() + num);
    if($('.area').val()){
      rebus();
    }
  }
...
$('.slider__button--toggle').click(function() {
  if($('.area').val()){
  $('.area-wrap').toggleClass("area-wrap--active");
  $('.start-date').toggleClass("start-date--hide");
  $('.area').toggleClass("area--active");
  $('.days-wrap').toggleClass("days-wrap--active");
  $('.calendar').toggleClass("calendar--hide");
  $('.wrap span').toggleClass("plus");
  }
});
...
$('.datepicker--content').addClass("key--animate");
...
$('.holydays__close').click(function() {
  $('.holydays').removeClass("holydays--show");
});



$('.attention__close').click(function() {
  $('.attention').removeClass("attention--show");
});
